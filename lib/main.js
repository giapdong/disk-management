...
myEmitter.on('add-event', () => {
    countInterval = 0;
});

myEmitter.once('count-down', () => {
    console.log('count-down started!')
});

/**
 * Read element into dir with path: pathNode
 * @param {String} pathNode Path to directory
 */
async function readDirPromise(pathNode) {
    return new Promise((resolve, reject) => {
        fs.readdir(pathNode, 'utf-8', function (err, data) {
            return resolve(data)
        })
    })
}
...
/**
 * Read stat of element by fs.stat by Promise
 * @param {String} pathNode Path to directory
    ...
    return new Promise(async (resolve, reject) => {
        ...
        result.forEach(async (element) => {
            ...
            if (stats.name != "Error") {
                ...
                rootNode.addChild(n);
                myEmitter.emit('add-event');
                myEmitter.emit('count-down')
...
async function Scan(root = __dirname, big = 1000) {
    ...
    let pathJSON = path.join(scanDir, `${year}-${month}-${date}_${hour}.${minutes}.${second}.log`)


    var obj = { time: currentDate, big_directory: dataScanBig, root: HierachyTree };
    ...
    console.timeEnd('Disk-management')
}
