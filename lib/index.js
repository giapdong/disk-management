...
emitter.on(EventType.ScanDone, async () => {
  ...
  console.log(`Total storage: ${myConst.bytesToSize(HierachyTree.storage)}`);
  ...
  if (dirInfo && dirInfo.name != "Error") {
...

    {
      ...
      _.map(dirInfo, (element) => {
        promise.push(readStatPromise(pathNode, element));
      });
...

    {
      let promise = [];
      _.map(data, (item) => {
        promise.push(actionEachItem(rootNode, pathNode, item.name, item.data));
      });
